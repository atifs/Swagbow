// Taken from http://www.strchr.com/expression_evaluator
...
private:
	...
	double ParseAtom(EVAL_CHAR*& expr) {
		...
		//First check if it is an exponent : Added by Cyrus
		if( *end_ptr == '^' ) {
			expr = end_ptr + 1;
			double power = strtod(expr, &end_ptr);
			expr = end_ptr;
			return pow(res, power);
		}
	...
	double ParseFactors(EVAL_CHAR*& expr) {
		...
		for(;;) {
			...
			if(op == '/') {
				...
				num1 *= num2;
			else if(op == '%') //Cyrus: Added in modulus
				num1 = fmod(num1, num2);
		}
	}
